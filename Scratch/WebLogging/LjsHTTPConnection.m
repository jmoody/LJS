...
@implementation LjsHTTPConnection

@synthesize httpFileLogger;

- (void) dealloc {
  [httpFileLogger release];
  [super dealloc];
}


- (id <DDLogFileManager>)logFileManager {
	if (self.httpFileLogger == nil) {
    LjsHTTPLogFileManager *fileManager = [[[LjsHTTPLogFileManager alloc] init] autorelease];
    self.httpFileLogger = [[[DDFileLogger alloc] initWithLogFileManager:fileManager] autorelease];    
    self.httpFileLogger.maximumFileSize = 1024 * 512;    // 512 KB
    self.httpFileLogger.rollingFrequency = 60 * 60 * 24; //  24 Hours
    self.httpFileLogger.logFileManager.maximumNumberOfLogFiles = 4;
    [DDLog addLogger:self.httpFileLogger];
  }
  return self.httpFileLogger.logFileManager;
}
